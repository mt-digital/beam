    ...
    case ResolvedParkingResponses(triggerId, xs) =>
      ...
      xs.foreach { case (id, resp) =>
        ...
        if (resp.stall.chargingPointType.isDefined) {
          chargingNetworkManager ! ChargingPlugRequest(0, veh, resp.stall, 
                                                       household.getMemberIds().get(0),  // use first household member id as stand-in.
                                                       triggerId)
        }
        ...
        List[Future[(Id[BeamVehicle], ParkingInquiryResponse)]] = 
          ...
          vehicles.toList.map { case (id, vehicle) =>
            if (vehicle.isBEV | vehicle.isPHEV) {
              (chargingNetworkManager ? ParkingInquiry.init(SpaceTime(homeCoord, 0), "charge", triggerId = triggerId))
